...
import 'package:ncm_api/utils/cookie.dart';
import 'package:ncm_api/utils/encrypt.dart';
...
// 主管理器
class SnowfluffMusicManager with AlbumApi, PlaylistApi, RecommendApi, SongApi, TopApi, UserApi, ArtistApi, SearchApi {
  static final SnowfluffMusicManager _instance = SnowfluffMusicManager._internal();
  factory SnowfluffMusicManager() => _instance;
  late Dio _dio;
  static late CookieJar cookieJar;
  SnowfluffMusicManager._internal();
  Future<void> init({required String cookiePath, bool debug = false}) async {
    cookieJar = await MusicFileCookieJar.create(cookiePath: cookiePath);
    _initDio();
  }
  void _initDio() {
...
class MusicApiInterceptors extends InterceptorsWrapper {
  ...
  Future<List<Cookie>> getCookies({Uri? host}) async {
    ...
    return (SnowfluffMusicManager.cookieJar).loadForRequest(host);
  }
  ...
  Future<void> deleteAllCookie() async {
    await (SnowfluffMusicManager.cookieJar as PersistCookieJar).deleteAll();
  }
  // 三类ua的handler
  ...
  void _handleWeApi(RequestOptions options) {
    ...
    // weApi方式请求body需要csrfToken字段，来自登录请求set-cookie
    String csrfToken = '';
    csrfToken = RegExp(r'_csrf=([^(;|$)]+)').firstMatch(options.headers[HttpHeaders.cookieHeader] ?? '')?.group(1) ?? '';
    if (csrfToken.isEmpty) {
      // map可能是<String,Int>，默认转换为<String,dynamic>
  ...
  void _handleEApi(RequestOptions options, List<Cookie> cookies, EncryptType type) {
    ...
    header['requestId'] = '${DateTime.now().microsecondsSinceEpoch}${Random().nextInt(1000).toString().padLeft(4, '0')}';
    options.data = Map.from(options.data);
